package com.example.gearshop.controller;

import com.example.gearshop.model.SanPhamPSU;
import com.example.gearshop.service.SanPhamPSUService;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Controller;
import org.springframework.ui.Model;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.RequestMapping;

import java.util.Arrays;
import java.util.List;

@Controller
@RequestMapping("/sanphampsu")
public class SanPhamPSUController {

    @Autowired
    private SanPhamPSUService psuService;

    @GetMapping("/{sanPhamID}")
    public String getPSUBySanPhamID(@PathVariable Integer sanPhamID, Model model) {
        // Lấy chi tiết sản phẩm PSU theo sanPhamID
        SanPhamPSU psu = psuService.findBySanPhamID(sanPhamID);

        // Kiểm tra nếu sản phẩm không tồn tại hoặc không phải PSU
        if (psu == null || psu.getSanPham().getLoaiSanPham().getId() != 6) {
            return "redirect:/error"; // Chuyển hướng đến trang lỗi nếu không tìm thấy sản phẩm
        }

        // Tách chuỗi mô tả thành danh sách
        List<String> motaList = psu.getMota() != null
                ? Arrays.asList(psu.getMota().split("\\|"))
                : null;

        // Thêm dữ liệu vào model
        model.addAttribute("sanPham", psu.getSanPham());
        model.addAttribute("sanPhamPSU", psu);
        model.addAttribute("motaList", motaList);
        model.addAttribute("loaiSanPham", "PSU");

        // Trả về giao diện chung
        return "clientTemplate/chitietsanpham";
    }
}
